...
test('home page visual baseline', async ({ page }) => {
  ...
  await stableScreenshot(page, 'home-page.png');
});
...
test('report page visual baseline', async ({ page }) => {
  await loginAs(page, 'volunteer-token');
  await page.goto('/report');
  await expect(page.getByRole('heading', { name: 'Report Flood' })).toBeVisible();
  await stableScreenshot(page, 'report-page.png');
});

test('rescue queue visual baseline', async ({ page }) => {
  await loginAs(page, 'volunteer-token');
  await page.goto('/rescue');
  await expect(page.getByRole('heading', { name: 'Rescue Dashboard' })).toBeVisible();
  await stableScreenshot(page, 'rescue-page.png');
});
...
test('admin dashboard visual baseline', async ({ page }) => {
  ...
  await stableScreenshot(page, 'admin-page.png');
});
